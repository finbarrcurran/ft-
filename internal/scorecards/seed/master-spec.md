...
- **Master Spec is bumped on every FT commit** (set 2026-05-19). Patch version (v1.7.1, v1.7.2) for polish; minor (v1.8) for new features or substantive refactors; major (v2.0) for breaking architecture changes. Each bump: (a) new row in §13, (b) `git commit` includes the spec change, (c) live DB updated via `UPDATE sector_scorecards … WHERE code='master-spec'` after deploy. Commits that touch only master-spec.md don't recursively bump.
