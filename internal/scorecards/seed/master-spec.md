...
>
> **Last meaningful overhaul:** 2026-05-30 (PM) — Spec 9l v0.5 patch + batch 2 ingest. 8 locked theses across 5 adapters; 9 cascade rows; first protocol_host cascade (Infrastructure ETH→LINK) and first hybrid coin (POL with L2 secondary advisory). DeFi/DePIN/RWA adapters locked but no theses yet (calibration gap). Earlier (AM): v0.4 patch + Phase 1 ship gate (8/10 acceptance criteria green; D25 Scoring Engine modal + cron framework deferred to Phase 2).
