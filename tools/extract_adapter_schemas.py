...
def split_into_pillar_sections(md: str) -> dict[str, str]:
    """
    Return a dict { 'Q1': <section text>, 'Q2': ..., 'Q9': ... } by splitting
    on '### Qn —' headings.
    """
    parts = re.split(r"(?=^###\s+Q\d+\s)", md, flags=re.MULTILINE)
    out: dict[str, str] = {}
    for p in parts:
        m = re.match(r"^###\s+Q(\d+)\s", p)
        if not m:
            continue
        n = int(m.group(1))
        if 1 <= n <= 9:
